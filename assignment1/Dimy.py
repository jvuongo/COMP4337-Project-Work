...
DBF_TIMER = 90 # 90s -> use 20 for testing
QBF_TIMER = 540 # 9min = 540s -> use 40 for testing

ecdh = generate_ephID_ECDH() # Generating ECDH curve for key
private_key_hash = 0
old_hash = 0
...
# Flag used to exit Dimy.py (and exit while loops) when user has COVID
exit_program = False
...
def create_new_ephID(ecdh):
    # Task 1: Generating ePHID
    new_ephid = generate_ephID_public_key(ecdh)

    # Add padding to make key 16 bits long
    hexlify_id = hexlify(new_ephid) + "00".encode()
    new_ephid = unhexlify(hexlify_id)
    print_id(new_ephid) # Printing ephID
    ...
    # Task 2: Create 5 chunks to broadcast
...
def compute_encID(ecdh, key):
    ...
    new_key = hexlify_key[:-2] # Removing the 00 padding
    new_key = new_key.encode()
    print(f"Original Key: {new_key}")
    unhexlify_key = unhexlify(new_key)
...
# Generating and Broadcasting EphID to other clients
def udp_broadcaster():
    ...
    # TASK 1 + 2: Create a new ephID + prepare n chunks
    ...
    while not exit_program:
        ...
        if curr_time > broadcast_timer:
            # TASK 3: Broadcasting n shares @ 1 unique share per 3s
            broadcast_string = str(broadcast_chunks[0][0]) + '|' + hexlify(broadcast_chunks[0][1]).decode() +'|'+ private_key_hash
            
            # TASK 3A: Message drop mechanism with probability 0.5
            ...
            # Update broadcast_timer
            broadcast_timer += BROADCAST_TIMER

        # TASK 1 (cont.): Generate a new ephID every 15 seconds
    ...
    # Exit when user has COVID and CBF is sent
    exit(1)

# Receiving other client's EphID
def udp_receiver():
    ...
    # Dictionary = {private_key_hash : [list of SSS received]}
    received_chunks = {}
    while not exit_program:
        ...
        if (recv_hash == private_key_hash or recv_hash == old_hash):
            # Private key hash has been received before 
            continue
        else:
            # Get receive index and chunk
            recv_index_chunk = (recv_index, unhexlify(recv_chunk.encode()))
            
            # Add chunk into dictionary
            ...
            if num_recv_shares >= 3:
                # All chunks were received, combine SSS
                ...
                # TASK 4-B: Reconstructed EphID hash value is the same as hash value received
                if (new_hash == recv_hash):
                    print("Hash has been identified, Computing EncID")

                    # Task 5-A Computing shared secret EncID using DH key mechanism
    ...
    exit(1) # Exit program if user is COVID positive

# Sending QBF to server
def tcp_sender():
...
# Thread to start broadcasting chunks
udp_broadcaster_thread = threading.Thread(name = "ClientUDPBroadcaster", target = udp_broadcaster)
udp_broadcaster_thread.start()
